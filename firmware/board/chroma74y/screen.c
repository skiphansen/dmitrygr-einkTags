...
#ifdef ATC1441_LUT
static const uint8_t __code  lut00[] = {
   0x01,0x05,0x00,0x01,0x07,0x01,0x06,0x3C,0x01,
   0x02,0x00,0x30,0x3C,0x01,0xDD,0x00
};
...
struct Atc1441Luts {
   ...
   {LUT_CMD_111,lut25,sizeof(lut25)},  // yellow
   {0}                                 // End of table
};
#endif
...
static void screenInitIfNeeded(__bit forPartial)
{
   ...
   int8_t i;
   int16_t j;
   pr("Init ATC1441 LUTs\n");
   for(i = 0; gAtc1441Luts[i].Cmd != 0; i++) {
      einkSelect();
      pr("Cmd 0x%x, len %d\n",gAtc1441Luts[i].Cmd,
         gAtc1441Luts[i].LutLen);
      ...
      for(j = 0; j < gAtc1441Luts[i].LutLen; j++) {
         ...
         // Empty table
            ...
            screenPrvSendData(gAtc1441Luts[i].Lut[j]);
         }
      }
      einkDeselect();
   }
