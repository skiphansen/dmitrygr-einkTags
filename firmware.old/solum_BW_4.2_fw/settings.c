...
static void settingsPrvDoWriteAtLocation(uint32_t addr, struct Settings* settings)
{
	...
	qspiWrite(false, addr, settings, sizeof(struct Settings));
}
...
void settingsPrvDoWrite(struct Settings* settings)
{
	...
	if (mCurSettingsAddr) {
		...
		qspiRead(0, mCurSettingsAddr, &sh, sizeof(struct SettingsHeader));
...
void settingsRead(struct Settings* settings)
{
	...
	if (bestAddr) {
		qspiRead(0, bestAddr, &sh, sizeof(struct SettingsHeader));	//to get size
		qspiRead(0, bestAddr, settings, sh.structSize);
