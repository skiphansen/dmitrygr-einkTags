...
##
## Copyright (C) 2025 Skip Hansen <skip@gfrn.org>
...
class Decoder(srd.Decoder):
    ...
    id = 'spi_epd'
    name = 'SPI_epd'
    longname = 'Electronic Paper display in 8 bit mode with C/D bit'
    ...
    optional_channels = (
        ...
        {'id': 'D/C', 'type': -1, 'name': 'D/C', 'desc': 'Data/Command'},
    )
    options = (
    ...
    def reset(self):
        ...
        self.bitcount = 0
        self.misodata = self.mosidata = self.d_c = 0
        self.misobits = []
        self.mosibits = []
        self.ss_block = -1
        self.samplenum = -1
    ...
    def putdata(self):
        ...
        if self.have_miso:
            self.put(ss, es, self.out_ann, [1, ['@%02X' % self.misodata]])
        if self.have_mosi:
            if self.d_c:
                self.put(ss, es, self.out_ann, [0, ['@D: %02X' % self.mosidata]])
            else:
                self.put(ss, es, self.out_ann, [0, ['@C: %02X' % self.mosidata]])
    ...
    def handle_bit(self, miso, mosi, clk, cs, d_c):
        ...
        if self.bitcount == 0:
        # save D/C at start of command bits
            self.d_c = d_c
    ...
    def find_clk_edge(self, miso, mosi, clk, cs, d_c, first):
        ...
        self.handle_bit(miso, mosi, clk, cs, d_c)
    ...
    def decode(self):
        ...
        if not self.has_channel(4):
            raise ChannelError('D/C pin required.')
        self.have_cs = self.has_channel(3)
        self.have_d_c = self.has_channel(4)
        ...
        (clk, miso, mosi, cs,d_c) = self.wait({})
        self.find_clk_edge(miso, mosi, clk, cs, d_c, True)

        while True:
            (clk, miso, mosi, cs, d_c) = self.wait(wait_cond)
            self.find_clk_edge(miso, mosi, clk, cs, d_c, False)
